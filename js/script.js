(function($) {
	...
	jQuery(document).on('ready',function(){
		...
		function initPortfolio(){
			...
			var allOption='a#all-sample';$(afterFirst).addClass('hide');
			...
			$('.popup-image').magnificPopup({type:'image',removalDelay:160,callbacks:{beforeOpen:function(){this.st.image.markup=this.st.image.markup.replace('mfp-figure','mfp-figure mfp-with-anim');
		...
		function initClientsSliders(){
			...
			function rotateClients(){
				...
				var mapOptions={zoom:17,center:new google.maps.LatLng(51.5287352,-0.3817831),mapTypeId:google.maps.MapTypeId.ROADMAP,scrollwheel:false,disableDefaultUI:false};
				var myMap=new google.maps.Map(document.getElementById('myMap'),mapOptions);
				var normal=new MarkerWithLabel({position:myMap.getCenter(),icon:{path:google.maps.SymbolPath.CIRCLE,scale:0},map:myMap,labelAnchor:new google.maps.Point(10,10),labelClass:"map-label",draggable:false});
				...
				var mapOptions = {zoom:17,center:new google.maps.LatLng(51.5287352,-0.3817831),mapTypeId:google.maps.MapTypeId.ROADMAP,scrollwheel:false,disableDefaultUI:false,styles:styles};
				var myMap = new google.maps.Map(document.getElementById('myMap'),mapOptions);
				var dark = new MarkerWithLabel({position:myMap.getCenter(),icon:{path:google.maps.SymbolPath.CIRCLE,scale:0},map:myMap,labelAnchor:new google.maps.Point(10,10),labelClass:"map-label",draggable:false});
			}function initMail(){
